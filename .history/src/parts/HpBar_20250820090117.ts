...
export default class HpBar {
  ...
  constructor(
    ...
    this.barWidth = barHeight - 10; // use master height because the bar width is the same as the height
    ...
    // === Bar isi (warna) ===
    this.bar = scene.add.image(x - 40, y - 5, "hpBarImage");
    this.bar
  ...
  draw() {
    ...
    this.maskShape.fillRect(
      this.bar.x - barWidth / 2, // mulai dari kiri
      this.bar.y - barHeight / 2, // dari atas
      barWidth * percent,
      barHeight
    );
