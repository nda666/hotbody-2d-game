...
export default class Player {
  ...
  constructor(scene: Phaser.Scene, gender: Gender, x: number, y: number) {
    this.scene = scene;
    this.x = x;
    this.y = y;
    this.gender = gender;
  ...
  preload() {
    ...
    const skinPath = getCharacterAsset(this.gender, "skin", skinName);

    this.scene.load.spritesheet("player", skinPath, {
      frameWidth: 80,
      frameHeight: 64,
    });
    this.pant.preload();
  ...
  create() {
    ...
    this.hair = new Hair(this.scene, this.x, this.y, "hair", "female");
    this.pant = new Pant(this.scene, this.x, this.y, "pant", "female");
