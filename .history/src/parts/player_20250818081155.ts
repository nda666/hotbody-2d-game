...
export default class Player {
  ...
  private createAnimations(scene: Phaser.Scene) {
    ...
    scene.anims.create({
      key: "attack",
      frames: scene.anims.generateFrameNumbers("skin", { start: 50, end: 55 }),
      frameRate: 6,
      repeat: -1,
    });
    scene.anims.create({
    ...
    if (this.sprite.input) {
      this.sprite.input.on("pointerdown", () => {
        if (!this.isAttacking) {
          ...
          // TODO: di sini bisa panggil fungsi hit detection / spawn projectile
