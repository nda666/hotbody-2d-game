...
export default class Hair {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, gender: Gender) {
    this.hair = scene.add.sprite(x, y, "hair").setDepth(11).setScale(6);
    const hairAssets = import.meta.glob(
      "../assets/character/female_hair/*.png",
      {
        eager: true,
      }
    );
  ...
  static preload(scene: Phaser.Scene) {
    const hairName = "female_hair1.png";
    console.log(Object.keys(hairAssets));
    const hairPath = (
      hairAssets[`../assets/character/female_hair/${hairName}`] as any
    ).default;
