...
export default class Player {
  ...
  private createAnimations(scene: Phaser.Scene) {
    ...
    this.scene.input.on("pointerdown", () => {
      if (!this.isAttacking && this.sprite?.body?.blocked.down) {
        this.isAttacking = true;
        this.sprite?.play("attack");

        this.sprite?.once(Phaser.Animations.Events.ANIMATION_COMPLETE, (c) => {
  ...
  update() {
    ...
    if (this.isAttacking && onGround) {
      this.bodyParts.forEach((part) => {
        if (part.spriteName !== "skin" && part.sprite) {
          part.sprite.play(`${part.spriteName}_attack`, true);
        }
      });
      return;
    }
    ...
    // console.log("this.sprite.body.velocity.y", this.sprite.body.velocity);
    // Animation state
    ...
    this.bodyParts.forEach((part) => {
      if (part.spriteName !== "skin" && part.sprite) {
        part.sprite.setVelocityX(this.sprite.body.velocity.x);
